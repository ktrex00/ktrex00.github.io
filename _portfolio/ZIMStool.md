...
Flet was the eventual choice for the main application, but it cannot be packaged into a single .exe file without the same issues as PyInstaller. It also cannot be run from a shared drive, as the PyInstaller .exe could. In order to allow for seamless updates, I created a launcher app that would run the program locally after checking for any app updates and installing them. All of this is done in the background so that a user does not have to worry about the installation process.